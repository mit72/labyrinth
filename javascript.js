...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    let y = 1;
    let animating = false;
    let currentProgress = 0; // Keeps track of the interpolation progress
    ...
    function drawSmoothLine() {
        ...
        if (x < resitev.length - 2) {
            // Starting and target points for this segment
            ...
            // Clear a tiny portion of the canvas and draw the line segment
            ctx.beginPath();
            ...
            // Increment progress
            currentProgress += 0.02; // Adjust this value to control speed

            if (currentProgress >= 1) {
                // Move to the next point and reset progress
                x += 2;
                y += 2;
                currentProgress = 0;
            }

            requestAnimationFrame(drawSmoothLine); // Request the next frame
        } else {
